...
import { X, HistoryIcon, Loader2, Settings2 } from "lucide-react"
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
...
const HistorySidePanel = () => {
  const { historyContext, toggleShowHistory, clearHistory, selectHistoryItem } =
  ...
  const downloadHistory = () => {
    ...
    const history_json = historyContext.entries.map((entry: any) => {
      ...
      const text = EditorState.createWithContent(
        convertFromRaw(entry.editor.internalState)
      )
        .getCurrentContent()
        .getPlainText()
      return {
  ...
  return (
    <div className="flex flex-col h-full relative overflow-auto">
      ...
      >
        ...
        <div
          className="cursor-pointer inline m-2 align-middle lg:inline align-middle mb-1"
              ...
              <div key={unique_date}>
                ...
                {historyContext.entries
                  ...
                  .map((historyItem: any, index: number) => {
                    const isSelectedHistoryItem =
                      historyContext.current.timestamp ===
                        historyItem.timestamp &&
                      historyContext.current.editor.prompt ===
...
const PromptCompletionEditor = ({ showDialog }) => {
  ...
  const { historyContext, addHistoryEntry, toggleShowHistory } =
    useContext(HistoryContext)
    ...
    const completionCallback = ({ event, data, meta }) => {
      switch (event) {
        ...
        case "status":
          ...
          const skipCohereError =
            modelProvider === "cohere" && message === "[ERROR] 'text'"
  ...
  const Decorated = (props: any) => {
    ...
    if (entityData.message === props.decoratedText) {
      ...
      if (
        ...
        let popoverContent = (
          <div
            className="shadow-xl shadow-inner rounded-sm bg-white mb-2"
            data-container="body"
          >
                  ...
                  <li
                    ...
                    className={
                      item[0] === entityData.message
                        ? "bg-highlight-tokens w-full font-base text-white pl-4"
                        : "pl-4 text-bg-slate-800"
                    }
                  >
  ...
  useEffect(() => {
    if (status.message && status.message.indexOf("[QUEUE] ") === 0) {
      toast({
        ...
        description:
          "We're currently experiencing high load, your compeletion request is in a queue and will be compeleted shortly",
      })
      return
    }
