...
import { getDecoratedStyle } from "../lib/editor-styles"
...
const ModelCardStats = (props: any) => {
  ...
  const paragraphs = insertLineBreaks(errorMessage).map((words, index) => (
    <span className="block text-center" key={index}>
      {words}
    </span>
  ))
  ...
  return (
    <div className="flex text-xs text-gray-500">
      ...
      <span>
        ...
        {token_per_second === 0 || errorMessage
          ? ""
          : `${token_per_second} chars/s`}{" "}
      </span>
      <span className="flex-1"></span>
...
const ModelCard = forwardRef((props, ref) => {
  ...
  const { modelsStateContext, setModelsStateContext } =
    useContext(ModelsStateContext)
  const { parametersContext, setParametersContext } =
  ...
  if (completion.length > token_index.current) {
    let completion_slice = completion.slice(
      token_index.current,
      completion.length
    )
  ...
  const Decorated = (props: any) => {
    ...
    const style = getDecoratedStyle(
      entityData.modelProvider,
      showHighlightsRef.current
    )
    ...
    if (entityData.message === props.decoratedText) {
      let content = (
        <span
          style={style}
          key={children[0].key}
          data-offset-key={children[0].key}
        >
          {children}
        </span>
      )
      ...
      if (
        probabilitiesMap &&
        tokensMap[props.decoratedText] != undefined &&
        tokensMap[props.decoratedText].length > 0
      ) {
        let percentage = Math.min(
          tokensMap[props.decoratedText][1] / probabilitiesMap.simpleProbSum,
          1.0
        )
        ...
        let popoverContent = (
          <div
            className="shadow-xl shadow-inner rounded-sm mb-2"
            data-container="body"
          >
            <ul key={children[0].key} className="grid pt-4">
              {Object.entries(tokensMap).map((item, index) => {
                return (
                  <li
                    key={item + "-" + index + "-" + children[0].key}
                    className={
                      item[0] === entityData.message
                        ? "bg-highlight-tokens w-full pl-4"
                        : "pl-4"
                    }
                  >
            ...
            <div className="m-4 pb-4">
              <div className="text-base">
                Total: {probabilitiesMap.logProbSum} logprob on 1 tokens
              </div>
              <div className="text-xs">
                ({probabilitiesMap.simpleProbSum}% probability covered in top{" "}
                {Object.keys(probabilitiesMap.tokens).length} logits)
              </div>
            </div>
          </div>
        )
        content = (
          ...
          >
            <span
              style={custom_style}
              className={popoverOpen ? "font-bold" : ""}
              id={children[0].key}
              key={children[0].key}
              data-offset-key={children[0].key}
              onClick={() => {
                showProbabilitiesRef.current
  ...
  useEffect(() => {
    ...
    if (
      status.message &&
      status.message.indexOf("[ERROR] ") === 0 &&
      serverModelState !== "COMPLETED" &&
      serverModelState !== "IDLE"
    ) {
    ...
    <div className={`flex flex-col items-center h-48 m-2`}>
      <div className="flex justify  max-w-[100%]">
        <h3
          ...
          className={`select-none cursor-pointer text-gray-400 max-w-full whitespace-nowrap overflow-hidden`}
        >
          {model.name}
        </h3>
      </div>
      <div
        className="relative editor-container bg-black h-full w-full text-base flex"
...
function PromptArea({ showDialog }) {
  ...
  const [editorState, setEditorState] = React.useState(
    EditorState.moveFocusToEnd(
      EditorState.createWithContent(
        editorContext.internalState !== null
          ? convertFromRaw(editorContext.internalState)
          : ContentState.createFromText(editorContext.prompt)
      )
    )
  )
  ...
  return (
    ...
    >
      <div className=" border border-gray-900 overflow-auto h-36">
        <PromptEditor
          editorState={editorState}
          setEditorState={setEditorState}
        />
...
const ModelsCompletion = ({ showDialog }) => {
          ...
          switch (data) {
            ...
            case "Too many daily completions":
              showDialog({
                ...
                message:
                  "It seems you've reached your daily limit. Please try again tomorrow.",
              })
              break
  ...
  return (
    <div className={"grid mt-3 pr-1 md:grid-cols-2 lg:grid-cols-3"}>
      {" "}
      {modelsStateContext
...
const ParametersTable = () => {
    ...
    ["Model", "ML", "T", "TP", "TK", "FP", "PP", "RP"].map((header, index) => (
      <div
        className={`text-center font-normal ${
          index === 0
            ? "min-w-[150px] sticky top-[0] left-[0] z-[1]"
            : "min-w-[150px] flex-1"
        }`}
      >
        {header}
      ...
      <div className="flex-1">
        <div className="mx-4 relative flx items-center">
          ...
          <div
            ...
            style={{
              width: `${100 * (parameterValue / parameterDefaults.range[1])}%`,
            }}
          />
        </div>
  ...
  const generate_row = (modelState, model) =>
    [
      ...
      parameter === "name" ? (
        <div
          className={`bg-black text-center font-light min-w-[150px] max-w-[150px] left-[0px] z-[2] sticky whitespace-nowrap text-ellipsis overflow-hidden`}
        >
        ...
        <div className="bg-inherit text-center font-light pt-2 relative min-w-[150px] flex flex-1 items-center">
          {generate_table_line_graph(
            modelState.parameters[parameter],
            model.defaultParameters[parameter]
          )}
        </div>
      )
    )
  ...
  return (
    <div>
      ...
      number_of_models_enabled === 0 ? null : (
      ...
      <NavBar tab="compare">
        <div className="align-middle mt-1">
          <div className="flex basis-full mb-2 lg:mb-0 space-x-2">
            <Sheet
              open={openParameterSheet}
              onOpenChange={setSaveOpenParameterSheet}
            >
